...
- **Dominio y persistencia**
  - Expandir `guardarOrden` para poblar fechas comprometidas, anticipo y saldo, garantizando que la orden refleje todo el ciclo operativo. -- LISTO
  - Calcular `costo_estimado` con los ítems actuales y permitir que `costo_final` se establezca al cierre; definir reglas de actualización automática del saldo en función de anticipos y pagos. -- LISTO CON OBSERVER
  - Validar que el cliente exista y que el dispositivo pertenezca al mismo cliente, sumado a la presencia de al menos un ítem; devolver errores descriptivos cuando falte alguno de estos requisitos. --LISTO
  - Encapsular la creación de la orden y los registros pivot (`orden_servicio`, `orden_producto`) en una transacción para evitar órdenes parcialmente creadas si surge un error en medio del proceso. -- LISTO
  - Añadir `casts` de fechas e importes en `OrdenTrabajo`, centralizar la enumeración de estados -- LISTO

- **Flujo de creación**
  - Mostrar en la ficha lateral los datos críticos del cliente seleccionado (RUT, teléfono, correo) para que el recepcionista no tenga que salir del flujo. --LISTO
  - Permitir crear clientes y dispositivos inline (modales rápidos) y resetear `selectedDeviceId` al cambiar de cliente para prevenir asociaciones incorrectas entre cliente y equipo. -- LISTO
