<div>
    ...
    <h1 class="text-xl font-semibold text-zinc-900 dark:text-zinc-100">Servicios</h1>

    <a href="{{ route('servicios.crear') }}" wire:navigate class="inline-flex items-center gap-2 px-3 py-2 rounded-md bg-zinc-900 text-white hover:bg-zinc-800 dark:bg-zinc-100 dark:text-zinc-900 dark:hover:bg-zinc-200">
    ...
    <div class="overflow-hidden rounded-lg border border-zinc-200 bg-white shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
        <table class="min-w-full divide-y divide-zinc-200 dark:divide-zinc-800">
            <thead class="bg-zinc-50 dark:bg-zinc-800/50">
                <tr>
                    <th class="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wide text-zinc-500">Servicio</th>
                    ...
                    <th class="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wide text-zinc-500">Precio</th>
            ...
            <tbody class="divide-y divide-zinc-200 bg-white dark:divide-zinc-800 dark:bg-zinc-900">
                @forelse ($servicios as $servicio)
                    <tr class="hover:bg-zinc-50 dark:hover:bg-zinc-800/40">
                        <td class="px-6 py-4 text-sm text-zinc-900 dark:text-zinc-100">{{ $servicio->nombre }}</td>
                        <td class="px-6 py-4 text-sm text-zinc-700 dark:text-zinc-300">{{ $servicio->categoria }}</td>
                        ...
                        <td class="px-6 py-4 text-right text-sm">
                            <span class="inline-flex items-center rounded-full px-2 py-1 text-xs font-medium {{ ($servicio->estado ?? 'activo') === 'inactivo' ? 'bg-zinc-100 text-zinc-600 dark:bg-zinc-800 dark:text-zinc-300' : 'bg-green-100 text-green-700 dark:bg-green-900/40 dark:text-green-300' }}">
                                {{ ucfirst($servicio->estado ?? 'activo') }}
                            </span>
                        </td>
                        <td class="px-6 py-4 text-right text-sm">
                            <a href="{{ route('servicios.editar', ['id' => $servicio->id]) }}" wire:navigate class="inline-flex items-center gap-1 text-zinc-700 hover:text-zinc-900 dark:text-zinc-300 dark:hover:text-zinc-100">
